  ...
  <body>
		...
		<table id="infoTable" class="table table-striped table-bordered table-condensed table-hover datatable">
		  ...
		  <s:iterator var="item" status="st" value="#list">
                <tr>		      
			      ...
			      <td><img height="60" width="120" src="<s:property value="#item.Picture"/>"></td>
			      <td><s:property value="#item.Picture"/></td>
			      <td><s:property value="#item.Category.CategoryName"/></td>
	...
	<script type="text/javascript">
		function search(){
			...
			window.location.href = "bookAction_query?keyWord=" + keyWord;
		}
		
		function del(id){
			if(confirm("Are you sure to delete the record?")){
				window.location.href = "bookAction_delete?id=" + id;
			}
		}
		
		function add(){
			window.location.href = "bookAction_showAdd";
		}
		
		function edit(id){
			window.location.href = "bookAction_showEdit?id=" + id;
		}
	</script>
  </body>
</html>
